...
public class Home extends AppCompatActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    String mainresult =
        "Day: " + Long.toString(orginaluser.howmanydays) + "\nWeekly Budget: $" + Float
            .toString(round(orginaluser.weeklyBudget(), 2)) + "\nAlready Spent: $" + Float
            .toString(round(orginaluser.getWeeklySpending(), 2)) + "\nAmount left: $" + Float
            .toString(round(orginaluser.weeklyBudget() - orginaluser.getWeeklySpending(), 2));
    if (orginaluser.weeklyBudget() - orginaluser.getWeeklySpending() < 0) {
      mainresult = mainresult + "\n\nThe overspending will bleed into next week!";
    } else if (orginaluser.weeklyBudget() - orginaluser.getWeeklySpending() < (
        orginaluser.weeklyBudget() * 0.1)) {
      mainresult = mainresult + "\n\nStop Spending! You only have 10% of budget left!";
    } else if (orginaluser.weeklyBudget() - orginaluser.getWeeklySpending() > 0) {
      mainresult = mainresult + "\n\nYou are saving $" + Float
          .toString(round(orginaluser.weeklyBudget() - orginaluser.getWeeklySpending(), 2))
          + " this week!";
    }
